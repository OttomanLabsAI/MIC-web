...
| v1.3 | Three tabs, always in view | A version bar now rides above every page: the original site, the new site, and the offer, each in its own tab that stays pinned at the top wherever the reader scrolls. The gold header link retires in its favour. |
| v1.4 | The original keeps its own window | The original-site tab now opens muslimsinconstruction.uk in a new browser tab, so the concept stays open beside it and the two can be compared side by side. |
